...
def resolve_team_map(session, season_id):
    """Fetch Yahoo's team list + Supabase's team list for this season, match by name."""
    ...
    sb_teams = supabase_get("teams", {"season_id": f"eq.{season_id}", "select": "id,team_name"})
    sb_by_name = {t["team_name"].strip().lower(): t["id"] for t in sb_teams}
    ...
    if unmatched:
        print("  ! WARNING: could not auto-match these Yahoo teams to baseball.teams by name:")
        for team_key, name in unmatched:
            print(f"      {team_key}  \"{name}\"  -- check for spelling drift between Yahoo and Supabase")

    print(f"  Resolved {len(team_map)}/{len(yahoo_teams)} teams automatically.")
    return team_map
